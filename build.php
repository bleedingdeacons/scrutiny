...
class PluginBuilder
{
    ...
    private string $pluginName = 'scrutiny';
    ...
    {
        $mainFile = $this->pluginDir . DIRECTORY_SEPARATOR . 'scrutiny.php';
        if (file_exists($mainFile)) {
            $content = file_get_contents($mainFile);
            if (preg_match('/Version:\s*([0-9.]+\s*\w*)/', $content, $matches)) {
                return trim($matches[1]);
            }
            if (preg_match("/define\s*\(\s*'SCRUTINY_VERSION'\s*,\s*'([^']+)'/", $content, $matches)) {
                return $matches[1];
            }
        }
        return '0.0.1';
...
Scrutiny WordPress Plugin Build Script
====================================
Platform: {$platform}
