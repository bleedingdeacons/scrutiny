...
Tested up to: 6.9
Stable tag: 1.32.3
Build date: 2026/09/05 16:43:39
Requires PHP: 8.0
...
Many fields *are* recorded outright, none of them personal data — each names a service status, a public entity or a setting rather than the member. The "Every tracked member field" table below lists them one by one; the service roles are the ones worth calling out here: the responder-certification stage (e.g. `Changed to Certified`), the member's home group (e.g. `Assigned to Thursday Big Book`), their intergroup position (e.g. `Removed from Telephone Liaison Officer`) and whether they are their home group's GSR, named by that group. Knowing which stage, group or position is the whole point of auditing these fields. GSR is compared as the group the member is GSR for, not as the flag alone, so carrying the role to a new home group is logged even though no flag changed. Each entry says in words what happened: `Assigned to New` when a role is taken, `Changed from Old to New` when it moves, `Removed from Old` when it is given up. A group or position that no longer resolves is recorded as `#<id>` so the entry stays traceable.
